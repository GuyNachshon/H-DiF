...
from k_diffusion.models import flags, flops
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x, pos, cond, cache=None):
        skip = x
        x = self.norm(x, cond)
        qkv = self.qkv_proj(x)
        ...
        if self.use_cache and cache is not None:
            ...
            # q, k, v: [n, heads, hw, e]; softmax scale 1.0 to match the fused path — the learned
            # cosine-sim scale already stands in for 1/sqrt(d_k), so no extra division here.
            logits = q @ k.transpose(-2, -1) + cache.bias(self, q.shape)
            attn = torch.softmax(logits, dim=-1)
            cache.store(self, attn.detach())
            x = attn.to(v.dtype) @ v
            x = rearrange(x, "n nh (h w) e -> n h w (nh e)", h=skip.shape[-3], w=skip.shape[-2])
        elif use_flash_2(qkv):
...
class GlobalTransformerLayer(nn.Module):
    ...
    def forward(self, x, pos, cond, cache=None):
        x = checkpoint(self.self_attn, x, pos, cond, cache)
...
class ImageTransformerDenoiserModelV2(nn.Module):
    ...
    def forward(self, x, sigma, aug_cond=None, class_cond=None, mapping_cond=None, cache=None):
        # Patching
        x = x.movedim(-3, -1)
        x = self.patch_in(x)
        ...
        x = self.mid_level(x, pos, cond, cache=cache)
